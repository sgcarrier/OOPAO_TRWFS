...
class PWFS_CL():
    ...
    def setup(self, traj, enable_custom_frames, ao_calib_file=None):
        ...
        self.numRemFramesPerBase = np.zeros((self.M2C_KL.shape[1]))
        if traj.enable_custom_frames == True:
            print("Finding optimal Frames to remove per mode")
            with silence():
                #self.dm.coefs = self.M2C_KL[:, :100] * stroke
                #self.tel * self.dm
                #bases = self.tel.OPD
        ...
        from OOPAO.calibration.InteractionMatrix import InteractionMatrix, interactionMatrix_withTR, interactionMatrix_withTR_no_negative
        ...
        if ((ao_calib_file) and (os.path.exists(ao_calib_file))):
            ...
            # custom_int_mat_D_plus, custom_int_mat_D = generateCustomInteractionMatrixFromDM(self.dm, self.tel, self.wfs, self.M2C_KL, self.numRemFramesPerBase, D_amp=stroke)

            ao_calib = interactionMatrix_withTR(ngs            = temp_ngs,
                                        atm            = self.atm,
                                        tel            = self.tel,
                                        dm             = self.dm,
                                        wfs            = self.wfs,
...
if __name__ == "__main__":
    ...
    env = Environment(trajectory='run_loops', filename='/home/cars2019/DATA/HDF/testing_123dsds4.hdf5',
    ...
    traj.f_explore(cartesian_product({'magnitude': [13.5, 14.0],
                                      'gainCL': [0.2],
