...
def compute_M2C_alt(telescope, atmosphere, deformableMirror, param=None, nameFolder=None, nameFile=None,
                remove_piston=False, HHtName=None, baseName=None, SpM_2D=None, nZer=3, SZ=None, mem_available=None,
                NDIVL=None, computeSpM=True, ortho_spm=True, computeSB=True, computeKL=True, minimF=False, P2F=None,
                alpha=None, beta=None, nmo=None, IF_2D=None, IFma=None, returnSB=False, returnHHt=False,
                recompute_cov=False, extra_name='', save_output=True):
    """
    - HHtName       = None      extension for the HHt Covariance file
    - baseName      = None      extension to the filename for basis saving
    - SpM_2D        = None      2D Specific modes [dim,dim,nspm], if None then automatic
    ...
    if recompute_cov is True:
        ...
        if save_output:
            try:
                ...
                aou.save(nameFolder + 'HHt_PSD_df_' + initName + 'r' + str(r0) + '_SZ' + str(SZ) + '.pkl',
                         [HHt, PSD_atm, df])
    ...
    if computeSB == True:
        ...
        if minimF == False:
            ...
            nSB = SB.shape[1]
            DELTA_SB = SB.T @ DELTA @ SB
            print('Orthonormality error for ' + str(nSB) + ' modes of the Seed Basis = ',
                  np.max(np.abs(DELTA_SB[0:nSB, 0:nSB] / tpup - np.eye(nSB))))
        ...
        if minimF == True:
            ...
            if nmo > SB.shape[1]:
                print('WARNING: Number of modes requested too high, taking the maximum value possible!')
                nmo = SB.shape[1]
            print('Orthonormality error for ' + str(nmo) + ' modes of the Seed Basis = ',
                  np.max(np.abs(DELTA_SB[0:nmo, 0:nmo] / tpup - np.eye(nmo))))
    ...
    if computeKL == True:
        ...
        # pdb.set_trace()
        DELTA_KL = KL.T @ DELTA @ KL
        print('Orthonormality error for ' + str(nmoKL) + ' modes of the KL Basis = ',
              np.max(np.abs(DELTA_KL[0:nmoKL, 0:nmoKL] / tpup - np.eye(nmoKL))))

        BASIS = np.asmatrix(np.zeros([nact, nmoKL], dtype=np.float64))
        # BASIS[:, 0:nspm] = SpM
        BASIS[:, :] = KL
        # if remove_piston == True:
        #     BASIS = np.asarray(BASIS[:, 1:])
