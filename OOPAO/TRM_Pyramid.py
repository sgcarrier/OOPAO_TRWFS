...
class TRM_Pyramid(Pyramid):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs["temporal_weights_settings"] is not None:
            self.temporal_weights_settings = kwargs["temporal_weights_settings"]
            del kwargs["temporal_weights_settings"]
            args_for_pyr = dict(kwargs)
        else:
            self.temporal_weights_settings = np.array([[1.0]*self.nTheta_user_defined])
            del kwargs["temporal_weights_settings"]
            args_for_pyr = dict(kwargs)

        super().__init__(**args_for_pyr)
    ...
    def wfs_measure(self, phase_in=None):
        ...
        if self.modulation == 0:
            if np.ndim(phase_in) == 2:
                ...
                self.pyramidSignalCube = np.zeros([len(self.temporal_weights_settings), self.nSignal, nModes])
                self.pyramidSignalCube_2D = np.zeros([len(self.temporal_weights_settings), self.validSignal.shape[0], self.validSignal.shape[1], nModes])
    ...
    def pyramid_transform(self,phase_in, phase=0):
        ...
        self.modulation_camera_em_phase.append(2 * np.pi - phase)
    ...
    def signalProcessing(self, cameraFrame=None):
        ...
        if self.postProcessing == 'fullFrame':
            ...
            fullFrameMaps_removed = np.zeros((len(self.temporal_weights_settings), fullFrameMaps.shape[0], fullFrameMaps.shape[1]))
            fullFrame_removed = np.zeros((len(self.temporal_weights_settings), np.sum(self.validSignal)))
            # self.saveGiftest('test_gif_no_remove.gif', cameraCube/np.max(cameraCube)*255)
            for i in range(len(self.temporal_weights_settings)):
                t_cube = cameraCube * self.temporal_weights_settings[i,:, np.newaxis, np.newaxis]
                # self.saveGiftest(f'test_gif_remove_{i}.gif', t_cube/np.max(cameraCube)*255)
                if self.referenceSignalCube_2D is None:
                    n_photons = np.sum(t_cube)
                    fullFrameMaps_removed[i, :, :] = (np.sum(t_cube, axis=0) / n_photons)
                else:
                    n_photons = np.sum(t_cube)
                    fullFrameMaps_removed[i, :, :] = (np.sum(t_cube, axis=0) / n_photons) - (self.referenceSignalCube_2D[i, :, :])
    ...
    def __mul__(self, obj):
        if obj.tag == 'detector':
            ...
            # obj.cube = obj.cube * (self.telescope.src.fluxMap.sum() / self.nTheta) / tmp3[:, np.newaxis, np.newaxis]
